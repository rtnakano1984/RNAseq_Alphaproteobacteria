...
# families <- data.frame(
# 	names=c("WRKY", "NAC", "HSF", "bHLH", "MYB", "MYB_related", "HD-ZIP", "YABBY", "AHL"),
# 	colours=c(c_dark_green, c_cudo_magenta, c_dark_brown, c_cudo_skyblue, c_blue, c_blue, c_dark_red, c_red, c_dark_yellow),
# 	stringsAsFactors=F)
...
# extract TF names
idx <- TF$Family %in% families$names
regulators <- rbind(TF[idx, c("Gene_ID", "Family")], AHL[, c("Gene_ID", "Family")]) %>% unique
...
write.table(as.data.frame(tab), file=paste(stat, "AME-k_means-zero_centered-9/DEG_TFs-enriched_motifs.txt", sep=""), col.names=F, row.names=T, quote=F, sep="\t")
...
# # extracting edges
# linkList   <- getLinkList(weight_mat, reportMax=2000)
# g <- graph.data.frame(linkList, directed=F)
# E(g)$weight <- linkList$weight

# # plot layout
# g_layout <- layout.fruchterman.reingold(g, niter=10000)
...
# saveRDS(g,        file=paste(processed_data, "GENIE3_network.RDS", sep=""))
# saveRDS(g_layout, file=paste(processed_data, "GENIE3_network_layout.RDS", sep=""))
...
# module detection
g_simply <- simplify(g)
fastgreedy <- cluster_fast_greedy(g_simply, merges=T, modularity=T,  membership=T, weights=E(g_simply)$weight)

# cluster_edge_betweenness <- cluster_edge_betweenness(g, merges=T, modularity=T,  membership=T, weights=E(g)$weight)

colours <- c(c_dark_red, c_red, c_cudo_magenta, c_green, c_blue, c_dark_green, c_dark_brown, c_dark_yellow, c_cudo_skyblue)

# plot
pdf(paste(fig, "Fig4B-GENIE3_all.pdf", sep=""), width=12, height=12)
	# plot.igraph(g1,
	...
	legend("topright", title="Regulators", legend=families$names, col=families$colours, pch=20, pt.cex=3, bty="n")
	...
	V(g)$community <- fastgreedy$membership
	plot.igraph(g,
		edge.width=E(g)$weight*10,
		vertex.size=V(g)$size,
		vertex.label=NA,
		vertex.color=colours[V(g)$community],
		layout=g_layout,
		main="GENIE3_WRKY_NAC-fast_greedy")
	legend("topright", title="Modules", legend=paste("module_", 1:length(sizes(fastgreedy)), sep=""), col=colours, pch=20, pt.cex=3, bty="n")

	# V(g)$community <- cluster_edge_betweenness$membership
	# plot.igraph(g,
	# 	edge.width=E(g)$weight*10,
	# 	vertex.size=V(g)$size,
	# 	vertex.label=NA,
	# 	vertex.color=colours[V(g)$community],
	# 	layout=g_layout,
	# 	main="GENIE3_WRKY_NAC-edge_betweeness")
	# legend("topright", title="Regulators", legend=family$names, col=family$colours, pch=20, pt.cex=3, bty="n")

dev.off() 


# export fastgreedy results
fastgreedy_membership <- c(membership(fastgreedy))
fastgreedy_membership_df <- data.frame(ID=names(fastgreedy_membership), module=paste("module_", fastgreedy_membership, sep=""), row.names=NULL, stringsAsFactors=F)
write.table(fastgreedy_membership_df, file=paste(stat, "GENIE3_WRKY_NAC-fast_greedy_membership.txt", sep=""), col.names=T, row.names=F, sep="\t", quote=F)


# export csv for metascape
max_N <- max(table(fastgreedy_membership_df$module))
metascape <- lapply(unique(fastgreedy_membership_df$module), function(x){
	idx <- fastgreedy_membership_df$module == x
	target <- fastgreedy_membership_df$ID[idx]
	out <- rep(NA, max_N)
	out[1:length(target)] <- target
	return(out)
}) %>% do.call(cbind, .)
colnames(metascape) <- unique(fastgreedy_membership_df$module)
write.table(metascape, file=paste(stat, "GENIE3_WRKY_NAC-fast_greedy_membership.for_metascape.csv", sep=""), sep=",", quote=F, col.names=T, row.names=F)
...
# plotting
pdf(paste(fig, "Fig2B-GENIE3_logFC.pdf", sep=""), width=12, height=12)
	genes <- V(g)$name
	i <- 0
...
# plot
pdf(paste(fig, "FigS2-GENIE3-k_means-zero_centered-9.pdf", sep=""), width=12, height=12)
...
ggsave(p, file=paste(fig, "Fig2C-GENIE3_centrality_scatter.pdf", sep=""), width=6, height=2.5, bg="transparent")

p1 <- p1 + geom_text_repel(segment.size=.5, size=3, show.legend=F)
p2 <- p2 + geom_text_repel(segment.size=.5, size=3, show.legend=F)
p <- p1 + p2 + plot_layout(ncol=2) & theme(plot.background=element_blank())
ggsave(p, file=paste(fig, "Fig2C-GENIE3_centrality_scatter.label.pdf", sep=""), width=6, height=2.5, bg="transparent")
...
ggsave(p3, file=paste(fig, "Fig2C-GENIE3_centrality_boxplot.pdf", sep=""), width=6, height=3, bg="transparent")
